...
int D_rip(int pos, char* val, stones pietre, stones *p_in_sol, int* curr_valore, int* max_valore, char *sol, char *sol1, int n, int K, int count, int* pf){
    ...
    if ((pos >= K) && (*pf==1) && (*curr_valore > *max_valore) && (is_Z_lower_S(p_in_sol))){
        for(i=0; i<K; i++){
