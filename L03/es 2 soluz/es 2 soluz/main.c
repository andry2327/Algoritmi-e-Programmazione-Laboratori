...
//  es 2 soluz
//
//  Created by Andrea Pellegrino on 22/10/2020.
//
...
lOMoARcPSD|5114792
Downloaded by Andrea Pellegrino (andry2327@gmail.com)
higher memory ----->
+----+----+----+----+ |0x01|0x00|0x00|0x00| +----+----+----+----+
x
| &x
...
void stampaCodifica1 (void *p, int size, int bigEndian); void stampaCodifica2 (void *p, int size, int bigEndian); void stampaMemoria (void *p, int size, int bigEndian); int checkBigEndian(void);
int main(int argc, char *argv[]) {
float af;
double ad;
long double ald;
int bigEndian = checkBigEndian();
printf ("Il sistema e': %s ENDIAN\n", bigEndian ? "BIG":"LITTLE"); printf("Dimensioni\n af = %d\n ad = %d\nald = %d\n", (int)sizeof(af), (int)sizeof(ad), (int)sizeof(ald));
printf("Inserire un valore (compatibile con float, double e long double): "); scanf("%f", &af);
printf("Ho letto: %f\n", af);
ad = (double)af;
ald = (long double)af;
printf("\nstampa della codifica float:\n");
/* stampaMemoria e' ridondante (non richiesta ma utile per 'vedere' i bit) */ stampaMemoria((void *)&af,sizeof(af),bigEndian);
stampaCodifica((void *)&af,sizeof(af),bigEndian);
printf("\nstampa della codifica double:\n");
/* stampaMemoria e' ridondante (non richiesta ma utile per 'vedere' i bit) */ stampaMemoria((void *)&ad,sizeof(ad),bigEndian);
stampaCodifica((void *)&ad,sizeof(ad),bigEndian);
printf("\nstampa della codifica long double:\n");
/* stampaMemoria e' ridondante (non richiesta ma utile per 'vedere' i bit) */ stampaMemoria((void *)&ald,sizeof(ald),bigEndian);
stampaCodifica((void *)&ald,sizeof(ald),bigEndian);
return 0;
}
int checkBigEndian(void) {
int test = 1; /* solo il byte meno significativo !=0, gli altri tutti ==0 */ char *pchar = (char *)&test; /* punta al byte di indirizzo più piccolo */ /* guarda il byte di indirizzo più piccolo: se ==0 Big Endian */
...
int i, j;
/* unsigned per vedere i byte come numeri da 0 a 255 (FF) */ /* puntatore usato come vettore di unsigned char */
...
/* riconosce i bit dal meno significativo e li accumula nel */ /* vettore di char (0/1) a partire dalla casella l; */
...
int i;
/* divide byte per 2 mediante shift di 1 posizione a destra */ for (i=0; i<8; i++, byte = byte>>1) {
...
/* riconosce i bit dal meno significativo e li accumula nel
vettore di char (0/1) a partire dalla casella l;
variante che non sposta i bit in "byte", ma li "guarda" usando un filtro: una maschera di bit da mettere in AND per evidenziare il bit scelto */
void separaBit3(unsigned char byte, char *bits, int l) {
    int i;
unsigned char mask;
/* divide byte per 2 mediante shift di 1 posizione a destra */ for (i=0,mask=0x01; i<8; i++, mask=mask<<1) {
...
case 8: return 11; /* double */ default: return 15; /* long double */ }
return 0; /* errore */
}

int dimensioneConPadding (int size, int bigEndian) {
/* se long double e little endian */
/* verifica eventuale padding (allineamento) e diminuisce size */ if (size>8 && !bigEndian) {
int size1;
...
/* assume padding a indirizzi alti e cerca byte con bit di segno */ for (size1=0; size1<size && pa[size1]==pb[size1]; size1++);
...
char bits[128]; /* non serve unsigned perche puo' valere solo 0/1 */ size = dimensioneConPadding (size, bigEndian);
/* scarica su bits i bit dal meno significativo */
    for (i=0; i<size; i++) {
separaBit(pchar[i], bits, i*8); }
nBits = size*8;
nExpBits = nBitEsponente(size);
printf("bit di segno : %d\n", (int)bits[nBits-1]); printf("esponente : "); //1000011100111000100011110101110 (original)
for (i=nBits-2; i>nBits-2-nExpBits; i--)                                   //01000011100111000100011110101110
printf("%d", (int)bits[i]); printf("\nmantissa : ");                       //100001110011100100011110101110
...
int i, nExpBits, nBits;
unsigned char *pchar = p;
size = dimensioneConPadding (size, bigEndian); nBits = size*8;
nExpBits = nBitEsponente(size);
printf("bit di segno : %d\n", estraiBit(pchar,nBits-1)); printf("esponente : ");
for (i=nBits-2; i>nBits-2-nExpBits; i--)
printf("%d", estraiBit(pchar,i)); printf("\nmantissa : ");
for (i=nBits-2-nExpBits; i>=0; i--)
printf("%d", estraiBit(pchar,i)); printf("\n");
}
